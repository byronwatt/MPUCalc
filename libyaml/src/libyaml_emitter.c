        ...
        int need_whitespace)
{
    ...
    while (string.pointer != string.end) {
        if (IS_ALPHA(string)
            ...
            while (width --) {
                int_value = *(string.pointer++);
                if (!PUT(emitter, '%')) return 0;
                if (!PUT(emitter, (int_value >> 4)
                            + ((int_value >> 4) < 10 ? '0' : 'A' - 10)))
                    return 0;
                if (!PUT(emitter, (int_value & 0x0F)
                            + ((int_value & 0x0F) < 10 ? '0' : 'A' - 10)))
                    return 0;
            }
        }
    }
    ...
    while (string.pointer != string.end)
    {
                ...
                || CHECK(string, '"') || CHECK(string, '\\'))
        {
            ...
            width = (octet & 0x80) == 0x00 ? 1 :
                    ...
                    (octet & 0xF0) == 0xE0 ? 3 :
                    (octet & 0xF8) == 0xF0 ? 4 : 0;
            int_value = (octet & 0x80) == 0x00 ? octet & 0x7F :
                    (octet & 0xE0) == 0xC0 ? octet & 0x1F :
                    (octet & 0xF0) == 0xE0 ? octet & 0x0F :
                    (octet & 0xF8) == 0xF0 ? octet & 0x07 : 0;
            for (k = 1; k < (int)width; k ++) {
                octet = string.pointer[k];
                int_value = (int_value << 6) + (octet & 0x3F);
            }
